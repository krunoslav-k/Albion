...
class UnitRecyclerAdapter(private var units: List<String>, private var lessonsLists: List<List<String>>, val fragmentManager: FragmentManager?): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(holder) {
            is UnitViewHolder -> {
                ...
                holder.createNestedRecyclerView(lessonsLists[position], fragmentManager)
            }
        }
    }
    ...
    class UnitViewHolder(itemView:View): RecyclerView.ViewHolder(itemView){
        ...
        lateinit var lessonRecyclerAdapter: LessonRecyclerAdapter

        fun bind(unit: String){
            unitNameTextView.text = unit
        }

        fun createNestedRecyclerView(lessons: List<String>, fragmentManager: FragmentManager?){

            lessonsRecyclerView.apply {
                layoutManager = LinearLayoutManager(itemView.context, LinearLayoutManager.HORIZONTAL, false)
                adapter = LessonRecyclerAdapter(lessons, fragmentManager)
            }
        }
